...
import { getEnvVisual } from '../components/Badges'
...
export default function CreateLock({ asPanel = false, onClose, onSuccess }: CreateLockProps = {}) {
  ...
  const envOptions: ChipOption<string>[] = environments.map((env) => ({
    value: env, label: getEnvironmentLabel(env) ?? env, visual: getEnvVisual(env),
  }))

  const neutralVisual = { bg: '#EFF4FF', text: '#1B3575', border: '#C2D0EF' }
  const resourceOptions: ChipOption<string>[] = resources.map((res) => ({
    value: res, label: res.charAt(0).toUpperCase() + res.slice(1), visual: neutralVisual,
  }))
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <ChipSelect ariaLabel="Environment" options={envOptions}
                        ...
                        onChange={(environment) => setFormData({ ...formData, environment })} />
                    </div>

                    <div>
                      <label className={labelCls} style={{ color: hud.onSurfaceVar }}>Resource</label>
                      <ChipSelect ariaLabel="Resource" options={resourceOptions}
                        value={formData.resource}
                        onChange={(resource) => setFormData({ ...formData, resource })} />
                    </div>
  ...
  if (asPanel) {
    return (
      <FormPanel
        ...
        title="New Lock"
        subtitle="Prevent concurrent deployments or operations"
        accent={hud.error}
        onClose={() => onClose?.()}
        onSubmit={handleSubmit}
        submitLabel={loading ? 'Creating...' : 'Create Lock'}
